...
本仓库提供了一个基于 FastAPI 的服务，用于管理大语言模型（LLM）训练项目及其生命周期。该服务覆盖了从定义训练项目，到发起、监控训练运行的核心工作流，并在相关响应中返回生成的工件概览。
...
- 聚焦训练项目的创建、查询与运行发起流程，保留关键日志与工件信息。
...
| `app.storage` | 模拟数据库 + 任务编排：生成 ID、维护项目状态、虚拟化日志/指标/工件。 | `InMemoryStorage`, `create_project`, `create_run`, `update_run_status` |
...
## API 参考
所有端点均返回 JSON 响应并使用标准 HTTP 状态码。除非另有说明，请以 JSON 格式提交请求体（创建运行无需请求体）。
...
- 当前版本聚焦运行的创建与监控，取消/恢复逻辑需由外部编排系统处理。
