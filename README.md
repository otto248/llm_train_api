...
## 存储说明
服务默认通过 `DatabaseStorage` 连接外部数据库实现持久化，连接串可使用环境变量 `TRAINING_DB_URL` 指定（未配置时退回到本地 `sqlite:///./training.db` 文件）。
所有项目、运行、日志与工件都会持久保存，服务重启后不会丢失数据。若需对接第三方提供的数据库，只需在部署时设置对应的连接地址即可。
